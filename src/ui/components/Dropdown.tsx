...
export const Dropdown = ({ trigger, children }: DropdownProps) => {
  ...
  return <div className='relative inline-block text-left'>
    <div>
      <Button className={(trigger.className) || ' px-4 py-2 text-white rounded-md hover:bg-neutral-700'} onClick={() => setExpanded(!expanded)}>{'label' in trigger && trigger.label}{'icon' in trigger && <Icon className='text-2xl' icon={trigger.icon} />}</Button>
    </div>
